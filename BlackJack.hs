...
value :: Hand -> Integer
value hand = reduceAces (initialValue hand) (numberOfAces hand)

-- Takes as input: a score and the number of aces that count as 11 in that score.
-- Returns: the maximum score without going over 21, if possible, by converting some aces from 11 to 1.
reduceAces :: Integer -> Integer -> Integer
reduceAces score numAces = if score > 21 && numAces > 0 then
                                reduceAces (score - 10) (numAces - 1) else score
...
winner guestHand bankHand = if not (gameOver guestHand) && (guestValue > bankValue || gameOver bankHand) then Guest else Bank
